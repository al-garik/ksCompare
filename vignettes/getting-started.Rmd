...
- `cmp$pattern_summary` -- recurring shapes per column (e.g.
  `case_only`, `constant_offset`) when `find_patterns = TRUE` was
  requested; otherwise an empty tibble with the usual columns.
- `cmp$unmatched_rows` -- full base-only / comp-only observations,
  capped by `max_unmatched_rows`.
...
- `cmp$verdict` -- executive headline used by `print(cmp)` and the
  HTML report.
...
`ks_comp_options()` controls semantics that apply to all columns:
...
- **Smart features:** see `vignette("smart-features")` for auto-key
  inference, duplicate-key handling, pattern detection, and SAS-style
  first / last unequal summaries.
