...
vignette: >
  %\VignetteIndexEntry{Pipeline gates}
...
- `max_schema_diffs` -- budget counted as base-only columns +
  comp-only columns + matched columns whose `kind` differs.
- `max_unmatched_rows` -- budget for `n_base_only_rows + n_comp_only_rows`.

## CI integration

In a CI script you usually want a summary written before the gate
fails so the failure is debuggable:
...
`ks_pointblank_step()` wraps `pointblank::specially()`, so the gate
participates in the agent's standard pass / warn / fail action levels.

## With arsenal

If your codebase already uses `arsenal::comparedf()`, the
`as_ks_comparison()` method re-runs the comparison via `ks_compare()`
